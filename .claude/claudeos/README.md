...
- Windows AutoRun / Task Scheduler（月〜土・プロジェクト別スケジュール・300分制限）が自律実行を担う
- `Start-ClaudeAutoTimeout.ps1` が各プロジェクトで Claude を 5 時間セッション起動
- `Register-AutoRunTask.ps1` でスケジュール登録・管理（メニュー 14）
- 5 時間上限で安全停止（session JSON 更新 → 終了報告）
- Agent Teams 会話の可視化
- README とドキュメントの継続更新
