...
- 日本語で対応・解説する
- コード内コメントは英語可
- 🎨 アイコン/emoji を多用する: Claude の応答・Agent 発話・README / docs / hook 出力で、見出し・表・箇条書き・ステータス・役割ラベルに積極付与する
- 意味のあるアイコンを選ぶ（成功=✅ / 警告=⚠️ / 調査=🔍 / メトリクス=📊 / 役割=👔💻🧪🔒 等）。可読性を損なう無意味な羅列だけは避ける
- emoji 描画不可端末では `CLAUDEOS_PLAIN_OUTPUT=1` によりプレーン出力へ fallback する
