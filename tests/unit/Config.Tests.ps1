...
Describe 'Get-RecentProject and Update-RecentProject' {
    ...
    Context 'Update-RecentProject のテスト' {
        ...
        It '重複が削除されること' {
            Update-RecentProject -ProjectName 'TestProject' -Tool 'codex' -Mode 'legacy' -HistoryPath $script:HistoryPath
            Update-RecentProject -ProjectName 'TestProject' -Tool 'codex' -Mode 'legacy' -HistoryPath $script:HistoryPath
            $result = Get-RecentProject -HistoryPath $script:HistoryPath
        ...
        It 'MaxHistory を超えた場合に古いエントリが削除されること' {
            1..12 | ForEach-Object {
                Update-RecentProject -ProjectName "Project$_" -Tool 'claude' -Mode 'legacy' -HistoryPath $script:HistoryPath -MaxHistory 3
            }
            $result = Get-RecentProject -HistoryPath $script:HistoryPath
